...
def build_resnet50_transfer(input_shape=(224,224,3), dropout=0.4):
    ...
    model = tf.keras.Model(inp, out, name="BreakHis_ResNet50_TL")
    return model, base
...
def main():
    ...
    # Para ResNet: tu normalizador usa preprocess_input(resnet50) y espera [0,255] :contentReference[oaicite:1]{index=1}
    config = DatasetConfig(
        img_size=(224,224),
        batch_size=32,
        augmentation_level=AugmentationLevel.ADVANCED,
        normalization_mode=NormalizationMode.RESNET,
    ...
    model, base = build_resnet50_transfer(input_shape=(*config.img_size, 3), dropout=0.4)
    ...
    cbs = [
        tf.keras.callbacks.ModelCheckpoint("cnn6_resnet50_best.h5", monitor="val_auc", mode="max", save_best_only=True, verbose=1),
    ...
    # --- Fase 2: fine-tuning parcial (últimas capas) ---
    base.trainable = True
    ...
    cbs2 = [
        tf.keras.callbacks.ModelCheckpoint("cnn6_resnet50_finetuned_best.h5", monitor="val_auc", mode="max", save_best_only=True, verbose=1),
    ...
    model.save("cnn6_resnet50_transfer_final.h5")
    print("\n💾 Modelo guardado: cnn6_resnet50_transfer_final.h5")
