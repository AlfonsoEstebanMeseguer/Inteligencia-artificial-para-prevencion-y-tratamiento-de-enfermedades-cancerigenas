...
# =========================================================
# CNN #3 – MULTI-SCALE (INCEPTION-LIKE) DESDE CERO
# =========================================================

def inception_block(x, filters, l2_reg, name):
    """
    Bloque Inception simplificado:
      - rama 1: 1x1
      - rama 2: 1x1 -> 3x3
      - rama 3: 1x1 -> 5x5 (aprox con 2x 3x3)
      - rama 4: maxpool -> 1x1
    Luego concat y proyección final.
    """
    # Rama 1: 1x1
    b1 = layers.Conv2D(filters, (1, 1), padding="same",
                       kernel_regularizer=regularizers.l2(l2_reg),
                       name=f"{name}_b1_1x1")(x)
    b1 = layers.BatchNormalization(name=f"{name}_b1_bn")(b1)
    b1 = layers.ReLU()(b1)

    # Rama 2: 1x1 -> 3x3
    b2 = layers.Conv2D(filters, (1, 1), padding="same",
                       kernel_regularizer=regularizers.l2(l2_reg),
                       name=f"{name}_b2_1x1")(x)
    b2 = layers.BatchNormalization(name=f"{name}_b2_bn1")(b2)
    b2 = layers.ReLU()(b2)
    b2 = layers.Conv2D(filters, (3, 3), padding="same",
                       kernel_regularizer=regularizers.l2(l2_reg),
                       name=f"{name}_b2_3x3")(b2)
    b2 = layers.BatchNormalization(name=f"{name}_b2_bn2")(b2)
    b2 = layers.ReLU()(b2)

    # Rama 3: 1x1 -> 3x3 -> 3x3 (aproxima 5x5 con 2 convs)
    b3 = layers.Conv2D(filters, (1, 1), padding="same",
                       kernel_regularizer=regularizers.l2(l2_reg),
                       name=f"{name}_b3_1x1")(x)
    b3 = layers.BatchNormalization(name=f"{name}_b3_bn1")(b3)
    b3 = layers.ReLU()(b3)
    b3 = layers.Conv2D(filters, (3, 3), padding="same",
                       kernel_regularizer=regularizers.l2(l2_reg),
                       name=f"{name}_b3_3x3a")(b3)
    b3 = layers.BatchNormalization(name=f"{name}_b3_bn2")(b3)
    b3 = layers.ReLU()(b3)
    b3 = layers.Conv2D(filters, (3, 3), padding="same",
                       kernel_regularizer=regularizers.l2(l2_reg),
                       name=f"{name}_b3_3x3b")(b3)
    b3 = layers.BatchNormalization(name=f"{name}_b3_bn3")(b3)
    b3 = layers.ReLU()(b3)

    # Rama 4: pool -> 1x1
    b4 = layers.MaxPooling2D((3, 3), strides=(1, 1), padding="same",
                             name=f"{name}_b4_pool")(x)
    b4 = layers.Conv2D(filters, (1, 1), padding="same",
                       kernel_regularizer=regularizers.l2(l2_reg),
                       name=f"{name}_b4_1x1")(b4)
    b4 = layers.BatchNormalization(name=f"{name}_b4_bn")(b4)
    b4 = layers.ReLU()(b4)

    # Concatenación multi-escala
    x = layers.Concatenate(name=f"{name}_concat")([b1, b2, b3, b4])

    # Proyección para controlar canales
    x = layers.Conv2D(filters * 2, (1, 1), padding="same",
                      kernel_regularizer=regularizers.l2(l2_reg),
                      name=f"{name}_proj_1x1")(x)
...
def build_cnn3_multiscale(
    input_shape=(224, 224, 3),
    num_classes=1,
    l2_reg=1e-4
):
    """
    CNN #3 – Inception-like multi-scale para histología (desde cero).
    Diseñada para mejorar AUC y especificidad vs CNN #2.
    """

    inputs = layers.Input(shape=input_shape, name="input_image")

    # Stem (captura temprana de texturas)
    ...
    # Bloques multi-escala + downsampling entre etapas
    ...
    model = models.Model(inputs, outputs, name="HistologyCNN_MultiScale")
    return model


# =========================================================
# UTILIDADES (idénticas a CNN #1 / #2)
# =========================================================
...
def main():
    ...
    model.save("cnn3_multiscale_final.h5")
    print("💾 Modelo guardado: cnn3_multiscale_final.h5")
