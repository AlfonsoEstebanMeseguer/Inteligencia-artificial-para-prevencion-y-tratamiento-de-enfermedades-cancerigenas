...
def parse_arguments():
    """
    Expone los hiperparámetros principales vía CLI (defaults intactos).
    """
    parser=argparse.ArgumentParser(description="Entrena la CNN #3 (multi-scale Inception-like) para BreakHis.")
    project_root=Path(__file__).resolve().parents[2]
...
def main():
    ...
    model=build_cnn3_multiscale((*config["img_size"],3),1)
    ...
    callbacks=[tf.keras.callbacks.EarlyStopping(monitor="val_loss",patience=ES_PATIENCE,restore_best_weights=True)
               ...
               ,tf.keras.callbacks.ModelCheckpoint(str(model_dir/"cnn3_multiscale_best.h5"),monitor="val_loss",save_best_only=True,verbose=1)]
