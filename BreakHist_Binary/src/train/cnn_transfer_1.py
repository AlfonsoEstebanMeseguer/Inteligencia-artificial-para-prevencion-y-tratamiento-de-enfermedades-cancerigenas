...
from src.models.models_definitions import build_resnet50_transfer
...
def parse_arguments():
    ...
    parser=argparse.ArgumentParser(description="Transfer learning con ResNet50 sobre BreakHis (binario).")
...
def main():
    ...
    # Para ResNet: tu normalizador usa preprocess_input(resnet50) y espera [0,255]
    ...
    model,base=build_resnet50_transfer((*config["img_size"],3),args.dropout)
    # entrenar cabeza
    ...
    callbacks=[tf.keras.callbacks.ModelCheckpoint(str(model_dir/"cnn6_resnet50_best.h5"),monitor="val_auc",mode="max",save_best_only=True, verbose=1),
    ...
    # fine-tuning (ultimas capas) 
    base.trainable=True
    # Congelar casi todo y afinar solo el "top"
    ...
    callbacks2=[tf.keras.callbacks.ModelCheckpoint(str(model_dir/"cnn6_resnet50_finetuned_best.h5"),monitor="val_auc",mode="max",save_best_only=True,verbose=1)
    ...
    model.fit(ds["train_ds"],validation_data=ds["val_ds"],epochs=args.epochs_ft,steps_per_epoch=ds["steps_per_epoch"]
              ,validation_steps=ds["val_steps"]
              ,class_weight=ds["class_weights"]
              ,callbacks=callbacks2,verbose=1)

    run_eval_and_artifacts(model,ds,args.threshold,None,str(model_dir/"cnn6_resnet50_predictions.npz")
                           ,True,"conv5_block3_out",str(model_dir/"cnn6_resnet50_transfer_final.h5"))
