...
"""
def DatasetConfig(img_size,batch_size,use_class_weights,normalization_mode,augmentation_level=ADVANCED,
    ...
    aug=augmentation_level.lower()
    if aug not in (NONE, LOW, MEDIUM, ADVANCED, EXPERT):
        raise ValueError(f"Nivel de augmentación '{augmentation_level}' no válido " f"(usar {NONE}, {LOW}, {MEDIUM}, {ADVANCED} o {EXPERT}) para multiclase")
    
    return {"img_size":tuple(img_size),"batch_size":batch_size,"use_class_weights":use_class_weights,"normalization_mode":norm,"augmentation_level":aug
            ,"augmentation_params":params.get(aug),"buffer_size":buffer_size,"cache":cache,"shuffle_train":shuffle_train,"prefetch":prefetch,"seed":seed}
